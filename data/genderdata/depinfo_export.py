...
LIST_URL = 'http://www.duma.gov.ru/about/history/convocations/1/?letter=%D0%90'            
...
class DepInfoParser:
	...
	def get_list(self, convoc=2):
		f = open('list_%d.txt' %(convoc), 'w')
		list_url = LIST_URL
		...
		for h in hrefs:
			...
			f.write((BASE_URL + h['href'] + '\n').encode('utf8'))
		f.close()
		pass

	def parse_lists(self, convoc=3):
		f = open('list_%d.txt' %(convoc), 'r')
		fd = open('deputies_cv%d.txt' %(convoc), 'w')
		for l in f:
			l = l.strip()
			u = urllib.urlopen(l)
			html = u.read()
			...
			trs = ddat.findAll('tr')	
			record = {}
			keys = ['fullname', 'url', 'faction_name', 'working']
			for tr in trs[1:]:
				tds = tr.findAll('td')
				record['fullname'] = tds[0].find('a').string
				record['url'] = BASE_URL + tds[0].find('a')['href']
				record['faction_name'] = tds[1].string
				coms = []
				hrefs = tds[2].findAll('li')
				for h in hrefs:
					coms.append(u"%s" %(h.string))
				record['working'] = u';'.join(coms)
				s = []
				for k in keys:
					s.append(record[k])
				sd = (u'\t'.join(s)).encode('utf8') + '\n'
				fd.write(sd)
				print sd
		pass
	...
	def map_new(self):
		...
		for l in f:
			...
			parts = l.split(u'\t')
			name  = parts[1]
			o = self.coll.find_one({'name' : name})
			if o is not None:
				o['is_gd6_real'] = True			
				print o['name'].encode('cp866')
				self.coll.save(o)
